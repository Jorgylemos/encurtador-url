...
public class Base62 {
	...
	static BigInteger decode(final String string, int bitLimit) {
		...
		if (!Pattern.matches("[" + DIGITS + "]", string)) {
			throwIllegalArgumentException("String '%s' contains illegal characters, only '%s' are allowed",
					string, DIGITS);

		}

		return IntStream.range(0, string.length())
				.mapToObj(index -> BigInteger.valueOf(charAt.apply(string, index))
				...
				.reduce(BigInteger.ZERO, (acc, value) -> {
					...
					if (bitLimit > 0 && sum.bitLength() > bitLimit) {
						throwIllegalArgumentException(
								"String '%s' contains more than 128bit information",
								string);
	...
	private static final BiFunction<String, Integer, Integer> charAt = (string, index) -> DIGITS
